...
def checkpoint(epoch):
    if not os.path.exists("./weights_DG"):          
        os.mkdir("./weights_DG") 
    if not os.path.exists("./weights_DG/train_DG"):          
...
def train(epoch):
    ...
    #     output_rgb_NR_noise, illumation_out_NR_noise = model(im_NR_noise, im_NR_noise_wb, im_NR_noise_gc, im_NR_noise_his)  
        ...
        # UIEB
        # 灰度世界：颜色均值一致性损失
        color_out_UIEB = color_loss(output_rgb*255.0)
        # 多样化色彩：颜色方差损失
        color_var_out_UIEB = colorVariance_loss(output_rgb*255.0)
        # LSUI
        # 灰度世界：颜色均值一致性损失
    ...
    #     # FFL = opt.FFL_weight * FFL_loss(output_rgb*255, im1*255)
        ...
        loss = color_out_UIEB - color_var_out_UIEB + color_out - color_var_out # 惩罚小方差，即鼓励方差大
    ...
    #     loss = MSE + Vgg + FFL + consisency_N_out + consisency_N_L
        ...
        loss_color = loss_color + color_out.item() + color_out_UIEB.item()
        loss_colorVariance = loss_colorVariance + color_var_out.item() + color_var_out_UIEB.item()
    ...
    #     loss_c_L = loss_c_L + consisency_N_L.item()
        ...
        if iter == train_len:
            ...
            if not os.path.exists(opt.val_folder+'training_WO-FR'):          
                os.mkdir(opt.val_folder+'training_WO-FR') 
            output_img.save(opt.val_folder+'training_WO-FR/res.png')
            gt_img.save(opt.val_folder+'training_WO-FR/gt.png')
            ...
            output_rgb_NR_img.save(opt.val_folder+'training_WO-FR/res_NR.png')
...
if __name__ == '__main__':  
    ...
    with open(f"./{opt.val_folder}/training_WO-FR/metrics_record_WO-FR_{now}.md", "w") as f:
