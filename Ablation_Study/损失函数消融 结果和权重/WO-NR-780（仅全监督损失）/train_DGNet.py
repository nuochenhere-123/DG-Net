...
def train(epoch):
    ...
    #     # FFL = opt.FFL_weight * FFL_loss(output_rgb*255, im1*255)
        ...
        loss = MSE + MSE_raw + Vgg + Vgg_raw + FFL + FFL_raw # 消融无监督损失，仅保留全监督
    ...
    #     loss_c_L = loss_c_L + consisency_N_L.item()
        ...
        if iter == train_len:
            ...
            if not os.path.exists(opt.val_folder+'training_WO-NR'):          
                os.mkdir(opt.val_folder+'training_WO-NR') 
            output_img.save(opt.val_folder+'training_WO-NR/res.png')
            gt_img.save(opt.val_folder+'training_WO-NR/gt.png')
...
if __name__ == '__main__':  
    ...
    with open(f"./{opt.val_folder}/training_WO-NR/metrics_record_WO-NR_{now}.md", "w") as f:
