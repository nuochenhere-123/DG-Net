...
def train(epoch):
    ...
    #     im_NR_noise_his = im_NR_noise_his.cuda()
        ...
        output_rgb, illumation_out = model(im1, im1_wb, im1_gc, im1_his)  
    # Step3 输出无参考结果 NR
    ...
    #     # FFL = opt.FFL_weight * FFL_loss(output_rgb*255, im1*255)
        ...
        loss = MSE + Vgg + FFL + color_out - color_var_out # 惩罚小方差，即鼓励方差大
    ...
    #     loss_c_L = loss_c_L + consisency_N_L.item()
        ...
        if iter == train_len:
            ...
            if not os.path.exists(opt.val_folder+'training_WO-Degradation'):          
                os.mkdir(opt.val_folder+'training_WO-Degradation') 
            output_img.save(opt.val_folder+'training_WO-Degradation/res.png')
            gt_img.save(opt.val_folder+'training_WO-Degradation/gt.png')
            ...
            output_rgb_NR_img.save(opt.val_folder+'training_WO-Degradation/res_NR.png')
...
if __name__ == '__main__':  
    ...
    with open(f"./{opt.val_folder}/training_WO-Degradation/metrics_record_WO-Degradation_{now}.md", "w") as f:
